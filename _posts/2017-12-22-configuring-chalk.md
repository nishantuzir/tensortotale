---
layout: post
title: "How to configure Tensor To Tale"
description: "Learn more about the config file for Tensor To Tale and how to set it up properly."
tags: [lstm, rl]
categories: [tensorflow, keras, matplotlib]
github: nishantuzir
docker: nishantuzir
---
...
Note: You do not _have_ to change anything. Tensor To Tale works out of the box. I do recommend changing the `_config.yml` to suit your needs.
...
* `local_fonts`: By default Tensor To Tale uses Google Fonts but if you wish to use local fonts this is the fallback.
...
Tensor To Tale uses Google Fonts by default. You can change the font in `_assets/javascripts/webfonts.js` and in `404.html`.
...
Don't forget that Tensor To Tale also supports local fonts if enabled in `config.yml`!

#### Footer

Changing the text in the footer is easy. It can be found in `_includes/footer.html`.

_If you have any questions about using or configuring Tensor To Tale please create an issue <a href="" title="here" rel="noreferrer noopener" target="_blank">here</a>!_
